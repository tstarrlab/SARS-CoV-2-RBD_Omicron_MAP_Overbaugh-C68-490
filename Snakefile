...
# Information on samples and barcode runs for SARS1 ----------------------
barcode_runs_SARS1 = pd.read_csv(config['barcode_runs_SARS1'])

# combination of the *library* and *sample* columns should be unique.
assert len(barcode_runs_SARS1.groupby(['library', 'sample'])) == len(barcode_runs_SARS1)

# *sample* should be the hyphen separated concatenation of
# *experiment*, *antibody*, *concentration*, and *sort_bin*.
sample_vs_expect_SARS1 = (
    barcode_runs_SARS1
    .query('experiment_type=="ab_selection"')
    .assign(concentration=lambda x: x['concentration'].astype(int),
            expect=lambda x: x[['experiment', 'antibody', 'concentration',
...
assert sample_vs_expect_SARS1['equal'].all(), sample_vs_expect_SARS1.query('equal != True')

# barcode runs with R1 files expanded by glob
barcode_runs_SARS1_expandR1 = (
    barcode_runs_SARS1
    .assign(R1=lambda x: x['R1'].str.split('; ').map(
                    lambda y: list(itertools.chain(*map(glob.glob, y)))),
            n_R1=lambda x: x['R1'].map(len),
...
assert barcode_runs_SARS1_expandR1['sample_lib'].nunique() == len(barcode_runs_SARS1_expandR1)
if any(barcode_runs_SARS1_expandR1['n_R1'] < 1):
    raise ValueError(f"no R1 for {barcode_runs_SARS1_expandR1.query('n_R1 < 1')}")
...
rule make_summary:
    ...
    input:
        ...
        get_mut_bind_expr_SARSr=config['mut_bind_expr_SARSr'],
        ...
        bind_expr_filters_SARS1=nb_markdown('bind_expr_filters_SARS1.ipynb'),
        aggregate_variant_counts_SARS1=nb_markdown('aggregate_variant_counts_SARS1.ipynb'),
        variant_counts_SARS1=config['variant_counts_SARS1'],
        counts_to_cells_ratio_SARS1=nb_markdown('counts_to_cells_ratio_SARS1.ipynb'),
        counts_to_cells_csv_SARS1=config['counts_to_cells_csv_SARS1'],
        counts_to_scores_SARS1=nb_markdown('counts_to_scores_SARS1.ipynb'),
        escape_scores_SARS1=config['escape_scores_SARS1'],
        escape_fracs_SARS1=config['escape_fracs_SARS1'],
        call_strong_escape_sites_SARS1=nb_markdown('call_strong_escape_sites_SARS1.ipynb'),
        strong_escape_sites_SARS1=config['strong_escape_sites_SARS1'],
        escape_profiles_SARS1=nb_markdown('escape_profiles_SARS1.ipynb'),
        output_pdbs_SARS1=nb_markdown('output_pdbs_SARS1.ipynb'),
        make_supp_data_SARS1=nb_markdown('make_supp_data_SARS1.ipynb'),
    output:
    ...
    run:
        ...
        with open(output.summary, 'w') as f:
            ...
            2. Count variants and then aggregate counts for
               ...
               [Omicron_BA2]({path(input.aggregate_variant_counts_Omicron_BA2)}), and
               [SARS1]({path(input.aggregate_variant_counts_SARS1)})
               to create variant counts files for [Wuhan_Hu_1]({path(input.variant_counts_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.variant_counts_Omicron_BA2)}), and
               [SARS1]({path(input.variant_counts_SARS1)}).

            3. Analyze sequencing counts to cells ratio for [Wuhan_Hu_1]({path(input.counts_to_cells_ratio_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.counts_to_cells_ratio_Omicron_BA2)}) and 
               [SARS1]({path(input.counts_to_cells_ratio_SARS1)}).
               this prints a list of any samples where this ratio too low. Also
               creates a CSV for [Wuhan_Hu_1]({path(input.counts_to_cells_csv_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.counts_to_cells_csv_Omicron_BA2)}) and 
               [SARS1]({path(input.counts_to_cells_csv_SARS1)}) with the
               sequencing counts, number of sorted cells, and ratios for
               all samples.

            4. Calculate escape scores from variant counts for [Wuhan_Hu_1]({path(input.counts_to_scores_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.counts_to_scores_Omicron_BA2)}) and 
               [SARS1]({path(input.counts_to_scores_SARS1)}).

            5. Call sites of strong escape for [Wuhan_Hu_1]({path(input.call_strong_escape_sites_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.call_strong_escape_sites_Omicron_BA2)}) and 
               [SARS1]({path(input.call_strong_escape_sites_SARS1)}).

            6. Plot escape profiles for [Wuhan_Hu_1]({path(input.escape_profiles_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.escape_profiles_Omicron_BA2)}) and 
               [SARS1]({path(input.escape_profiles_SARS1)}).

            7. Map escape profiles to ``*.pdb`` files using notebooks here for 
               [Wuhan_Hu_1]({path(input.output_pdbs_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.output_pdbs_Omicron_BA2)}) and 
               [SARS1]({path(input.output_pdbs_SARS1)}).
            
          8. Make supplementary data files for [Wuhan_Hu_1]({path(input.make_supp_data_Wuhan_Hu_1)}), 
               [Omicron_BA2]({path(input.make_supp_data_Omicron_BA2)}) and 
               [SARS1]({path(input.make_supp_data_SARS1)}),
               which are here for [Wuhan_Hu_1]({path(config['supp_data_dir_Wuhan_Hu_1'])}), 
               [Omicron_BA2]({path(config['supp_data_dir_Omicron_BA2'])}) and 
               [SARS1]({path(config['supp_data_dir_SARS1'])}). These include
...
rule make_supp_data_SARS1:
    input:
        config['escape_profiles_config_SARS1'],
        config['output_pdbs_config'],
        config['escape_fracs_SARS1'],
        config['escape_profiles_dms_colors_SARS1']
    output:
        nb_markdown=nb_markdown('make_supp_data_SARS1.ipynb'),
        outdir=directory(config['supp_data_dir_SARS1']),
    params:
        nb='make_supp_data_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"
        
rule output_pdbs_SARS1:
    input:
        config['escape_fracs_SARS1'],
        config['output_pdbs_config'],
    output:
        nb_markdown=nb_markdown('output_pdbs_SARS1.ipynb'),
        outdir=directory(config['pdb_outputs_dir_SARS1']),
    params:
        nb='output_pdbs_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"

rule escape_profiles_SARS1:
    """Make stacked logo plots of antibody escape profiles."""
    input:
        escape_fracs=config['escape_fracs_SARS1'],
        escape_profiles_config=config['escape_profiles_config_SARS1'],
        site_color_schemes=config['site_color_schemes'],
        wildtype_sequence=config['wildtype_sequence_SARS1'],
        mut_bind_expr=config['mut_bind_expr_SARSr'],
        strong_escape_sites=config['strong_escape_sites_SARS1'],
    output:
        nb_markdown=nb_markdown('escape_profiles_SARS1.ipynb'),
        escape_profiles_dms_colors=config['escape_profiles_dms_colors_SARS1'],
    params:
        nb='escape_profiles_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"

rule call_strong_escape_sites_SARS1:
    """Call sites of strong escape."""
    input:
        escape_fracs=config['escape_fracs_SARS1'],
    output:
        nb_markdown=nb_markdown('call_strong_escape_sites_SARS1.ipynb'),
        strong_escape_sites=config['strong_escape_sites_SARS1'],
    params:
        nb='call_strong_escape_sites_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"


rule counts_to_scores_SARS1:
    """Analyze variant counts to compute escape scores."""
    input:
        config['variant_counts_SARS1'],
        config['wildtype_sequence_SARS1'],
        # config['mut_bind_expr'],
        # config['variant_expr'],
        # config['variant_bind'],
    output:
        nb_markdown=nb_markdown('counts_to_scores_SARS1.ipynb'),
        escape_scores=config['escape_scores_SARS1'],
        escape_fracs=config['escape_fracs_SARS1'],
        escape_score_samples=config['escape_score_samples_SARS1'],
    params:
        nb='counts_to_scores_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"

        
rule counts_to_cells_ratio_SARS1:
    input:
        config['variant_counts_SARS1'],
        config['barcode_runs_SARS1'],
        config['wildtype_sequence_SARS1'],
    output:
        nb_markdown=nb_markdown('counts_to_cells_ratio_SARS1.ipynb'),
        counts_to_cells_csv_SARS1=config['counts_to_cells_csv_SARS1'],
    params:
        nb='counts_to_cells_ratio_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"


rule aggregate_variant_counts_SARS1:
    input:
        counts=expand(os.path.join(config['counts_dir_SARS1'],
                                   "{sample_lib}_counts.csv"),
                      sample_lib=barcode_runs_SARS1_expandR1['sample_lib']),
        fates=expand(os.path.join(config['counts_dir_SARS1'],
                                  "{sample_lib}_fates.csv"),
                     sample_lib=barcode_runs_SARS1_expandR1['sample_lib']),
        variant_table=config['bc_variant_lookup_SARS1'],
        wt_seq=config['wildtype_sequence_SARS1'],
        barcode_runs=config['barcode_runs_SARS1'],
    output:
        config['variant_counts_SARS1'],
        nb_markdown=nb_markdown('aggregate_variant_counts_SARS1.ipynb')
    params:
        nb='aggregate_variant_counts_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"


        
rule count_variants_SARS1:
    """Count variants for a specific sample."""
    input:
        variant_table=config['bc_variant_lookup_SARS1'],
        wt_seq=config['wildtype_sequence_SARS1'],
        r1s=lambda wildcards: (barcode_runs_SARS1_expandR1
                               .set_index('sample_lib')
                               .at[wildcards.sample_lib, 'R1']
                               ),
    output:
        counts=os.path.join(config['counts_dir_SARS1'], "{sample_lib}_counts.csv"),
        fates=os.path.join(config['counts_dir_SARS1'], "{sample_lib}_fates.csv"),
    params:
        sample_lib="{sample_lib}"
    run:
        # parse sample and library from `sample_lib` wildcard
        lib = params.sample_lib.split('_')[-1]
        sample = params.sample_lib[: -len(lib) - 1]
        assert sample == (barcode_runs_SARS1_expandR1
                          .set_index('sample_lib')
                          .at[params.sample_lib, 'sample']
                          )
        assert lib == (barcode_runs_SARS1_expandR1
                       .set_index('sample_lib')
                       .at[params.sample_lib, 'library']
                       )
        # initialize `CodonVariantTable` (used to get valid barcodes)
...
rule bind_expr_filters_SARS1:
    """QC checks on bind & expression filters from DMS data.
    """
    input:
    	file=config['mut_bind_expr_SARSr']
    output:
        nb_markdown=nb_markdown('bind_expr_filters_SARS1.ipynb')
    params:
        nb='bind_expr_filters_SARS1.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"
...
rule get_mut_bind_expr_SARSr:
    """Download sarbecovirus mutation ACE2-binding and expression from URL."""
    output:
        file=config['mut_bind_expr_SARSr']
    run:
        urllib.request.urlretrieve(config['mut_bind_expr_url_SARSr'], output.file)
